...
function formatarVistoPorUltimo(ultimoAcesso: string | null | undefined, status: StatusPresenca, eVoce: boolean): string {
  ...
  if (status === "ocupado") return "NÃ£o perturbe";
  ...
  if (diffMin < 60) return `Visto hÃ¡ ${diffMin} min`;
  if (diffHoras < 24) return `Visto hÃ¡ ${diffHoras} ${diffHoras === 1 ? "hora" : "horas"}`;
  if (diffDias === 1) return "Visto ontem";
  if (diffDias < 7) return `Visto hÃ¡ ${diffDias} dias`;
...
function PontoPresenca({ status, eVoce }: { status: StatusPresenca; eVoce: boolean }) {
  ...
  if (statusEfetivo === "ocupado") {
    return (
      <span className="absolute bottom-0 right-0 flex h-3.5 w-3.5 items-center justify-center rounded-full bg-papel ring-2 ring-papel shadow-xs" title="NÃ£o perturbe (Lendo em paz)">
        <span className="relative flex h-2.5 w-2.5">
...
function PaginaLeitores() {
  ...
  // Heartbeat e polling para sincronizar status de presenÃ§a em tempo real
  useEffect(() => {
    // Registra presenÃ§a ativa ao abrir a pÃ¡gina
  ...
  return (
    <div className="min-h-dvh pb-24">
      ...
      <main className="mx-auto max-w-2xl px-4 sm:px-6">
        ...
        <p className="mt-1 text-tinta-2">As estantes desta casa. Cada perfil mostra sÃ³ o que a pessoa deixou pÃºblico.</p>
        ...
        <div className="mt-8 space-y-4">
          {leitores.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-tinta-3 p-8 text-center">
              ...
              <p className="mt-1 text-sm text-tinta-2">Use o botÃ£o "+ Convidar" para convidar pessoas para a sua estante.</p>
            </div>
          ) : (
            [...leitores]
              .sort((a, b) => {
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-0.5 truncate text-sm text-tinta-2">
                        ...
                        {l.lendoAgora ? ` Â· lendo ${l.lendoAgora}` : ""}
                      </p>
                    </div>

                    {/* BotÃ£o de Excluir Leitor: VisÃ­vel apenas para a moderadora judaviluis (id === 1) e nÃ£o para ela mesma */}
      ...
      {/* Modal de ConfirmaÃ§Ã£o para Remover Leitor */}
        ...
        >
          ...
          >
            ...
            <p className="text-xs text-amora font-medium leading-relaxed bg-amora-clara/40 border border-amora/20 p-3 rounded-2xl">
              âš ï¸ Esta aÃ§Ã£o Ã© irreversÃ­vel. Todos os livros da estante, progresso de leitura, opiniÃµes e histÃ³rico de conquistas deste leitor serÃ£o excluÃ­dos permanentemente.
            </p>
            ...
            <div className="mt-6 flex items-center justify-end gap-3 pt-2">
              ...
              >
                {excluindo ? "Removendo..." : "Confirmar RemoÃ§Ã£o"}
              </button>
            </div>
          </div>
        </div>,
    ...
    >
      ...
      >
        ...
        <div className="overflow-y-auto p-6 space-y-6 flex-1">
          <p className="text-sm text-tinta-2 leading-relaxed">
            Gere um link e envie para quem vocÃª quer nesta casa. A pessoa cria a prÃ³pria conta e estante.
          </p>
          ...
          {limiteAtingido && (
            <p className="text-xs text-amora font-medium text-center">
              VocÃª jÃ¡ possui 3 convites pendentes e ativos. Revogue um deles ou aguarde expirar para gerar mais.
            </p>
          )}
